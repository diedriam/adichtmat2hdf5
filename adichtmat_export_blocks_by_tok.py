...
def adichtmat_export_blocks_by_tok(filename, tok_id = '', tok_longid='', tok_start='', tok_stop=''):
    ...
    if len(tok_id) > 0 & (len(tok_longid) > 0):
        toks = {tok_id: [tok_longid, tok_start, tok_stop] }
    else:
        toks = {
...
def adichtmat_export_blocks_by_tok2(filename, tok_id = '', tok_longid='', tok_start='', tok_stop=''):
    ...
    if len(tok_id) > 0 & (len(tok_longid) > 0):
        tokens = {
            'tok' : [
                { 'tok_id': tok_id , 'tok_longid': tok_longid, 'tok_start': tok_start, 'tok_stop':  tok_stop },
            ]
        } 
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print(argcount)
        if argcount == 2:
            adichtmat_export_blocks_by_tok2(sys.argv[1])
        if len(sys.argv) == 4:
        ...
        print('adichtmat_export_blocks_by_tok: missing argument filename')
        print('running in test ...')
        filename = '/Users/diedriam/DATA/DATA_Local/tmp/TomM/2021-12-14_080000_TomM_AD059P_Day0_tilt.mat'
        # adichtmat_export_blocks_by_tok2(filename, tok_id = 'VM', tok_longid='@Valsalva Maneuver')
